...
const TopHeader = ({ openMobile, toggleMobile }) => (
...
const SidebarNav = ({ pathname, onItemClick }) => (
    <nav className="p-4 space-y-2">
        {NAV.map(({ to, label, icon: Icon, testid }) => (
            <NavLink
                ...
                data-testid={testid}
                onClick={onItemClick}
                className={() => getNavItemClass(isPathActive(pathname, to))}
            >
...
const Sidebar = ({ open, pathname, onItemClick }) => (
    <aside
        className={`${open ? "block" : "hidden"} md:block w-full md:w-64 bg-[#DDD6FE] border-r-2 border-black md:sticky md:top-[73px] md:h-[calc(100vh-73px)] z-20`}
        data-testid="sidebar"
    >
...
const Layout = () => {
    ...
    return (
        <div className="min-h-screen flex flex-col" data-testid="app-root">
            <Marquee />
            <TopHeader openMobile={openMobile} toggleMobile={() => setOpenMobile(!openMobile)} />
